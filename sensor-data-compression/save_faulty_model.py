...
def main(args):
    ...
    if 0 < args.num_val_inputs <= val_input.shape[0]:
        ...
        raise RuntimeError(f"Improper configuration for 'num_val_inputs'. Max = {val_input.shape[0]}")
    ...
    for bit_i in range(*bit_flip_range_step):
        ...
        # Faulty kernel has been assigned to the model, so now we can save the
        # model as an HDF5 file
        f_autoencoder.save(f"./faulty_models/faulty_{args.ieu_model_id}_bit{bit_i}.h5")
        ...
        print(f"Time to run FI loop: {time.time() - fi_loop_start} seconds")
        break
