...
from train import load_data, normalize, build_model, split, train
...
def main(args):
    ...
    normdata = normalize_data(data_values)
    ...
    if model["ws"] == "":
        raise RuntimeError("No weights to preload into model!")
        # If no user input weights, then train the model
        # history = train(
        #     m_autoCNN,
        #     m_autoCNNen,
        #     train_input,
    ...
    # Evaluate the model
    fmodel = FModel(m_autoCNNen, args.model_ber)
